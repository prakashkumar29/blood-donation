...
import { MY_USERS_LIST, PROFILE } from "../../../../routes/routePaths";
...
import { labels, initialValues } from "../../../../constants/masters/myUsers";
...
function Form() {
  ...
  const previousPath = location?.state?.previousPath || PROFILE;
  ...
  const { refetch: getUpdatedDetails } = useQuery(
    ...
    () => getApiServices(getUserDetailByToken),
    {
      ...
      onSuccess: ({ data }) => {
        ...
        navigate(!!isUser ? previousPath : MY_USERS_LIST);
      },
    }
  );
  ...
  const { refetch: createUser } = useQuery(
    ...
    () => {
      ...
      return postApiServices(USER, formValues);
    },
    {
      enabled: false,
      onSuccess: ({ data }) => {
        updateSeed(adminUsers);
        navigate(MY_USERS_LIST, {
          state: data?.id,
        });
  ...
  const { refetch: updateUser } = useQuery(
    ...
    () => {
      ...
      return updateApiServices(USER, editId, formValues);
    },
    {
      enabled: false,
      onSuccess: ({ data }) => {
  ...
  useQuery("user", () => getByIdApiServices(USER, editId), {
    enabled: !!editId,
    onSuccess: ({ data }) => {
      setValues({
        ...data,
  ...
  return (
    <>
      <BackNavigator
        title={!!isUser ? "PROFILE" : "USER"}
        navigateTo={!!isUser ? previousPath : MY_USERS_LIST}
      />
      <FormLayout>
        <StyledFormContainer>
          ...
          {editId && !isUser ? <AuditInfo details={values} /> : <></>}
        </StyledFormContainer>
      </FormLayout>
    </>
  );
